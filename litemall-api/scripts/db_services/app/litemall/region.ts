...
export class AppLitemallRegionService {
  ...
  static Upsert(
    ...
    return Process(
      `models.${AppLitemallRegionService.ModelID}.Upsert`,
      data,
      uniqueBy,
      updateColumns
    );
   ...
   * @returns
   */
  static InsertBatch(data: IAppLitemallRegion[]): number {
   ...
   * @returns
   */
  static Save(data: Partial<IAppLitemallRegion>): number {
   ...
   * @returns
   */
  static Update(key: number, line: Partial<IAppLitemallRegion>) {
    return Process(
  ...
  static UpdateWhere(
    query: YaoQueryParam.QueryParam,
    line: Partial<IAppLitemallRegion>
  ) {
    return Process(
  ...
  static DeleteAll() {
    return new Query('default').Run({
      sql: { stmt: `delete from ${AppLitemallRegionService.TableName}` }
    });
  }
