...
export class AppLitemallFootprintService {
  ...
  static Upsert(
    ...
    return Process(
      `models.${AppLitemallFootprintService.ModelID}.Upsert`,
      data,
      uniqueBy,
      updateColumns
    );
   ...
   * @returns
   */
  static InsertBatch(data: IAppLitemallFootprint[]): number {
   ...
   * @returns
   */
  static Save(data: Partial<IAppLitemallFootprint>): number {
   ...
   * @returns
   */
  static Update(key: number, line: Partial<IAppLitemallFootprint>) {
    return Process(
  ...
  static UpdateWhere(
    query: YaoQueryParam.QueryParam,
    line: Partial<IAppLitemallFootprint>
  ) {
    return Process(
  ...
  static DeleteAll() {
    return new Query('default').Run({
      sql: { stmt: `delete from ${AppLitemallFootprintService.TableName}` }
    });
  }
