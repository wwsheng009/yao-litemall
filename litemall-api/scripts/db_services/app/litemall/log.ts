...
export class AppLitemallLogService {
  ...
  static Upsert(
    ...
    return Process(
      `models.${AppLitemallLogService.ModelID}.Upsert`,
      data,
      uniqueBy,
      updateColumns
    );
   ...
   * @returns
   */
  static InsertBatch(data: IAppLitemallLog[]): number {
   ...
   * @returns
   */
  static Save(data: Partial<IAppLitemallLog>): number {
   ...
   * @returns
   */
  static Update(key: number, line: Partial<IAppLitemallLog>) {
   ...
   * @returns
   */
  static EachSave(data: IAppLitemallLog[], line: Partial<IAppLitemallLog>) {
    return Process(
  ...
  static DeleteAll() {
    return new Query('default').Run({
      sql: { stmt: `delete from ${AppLitemallLogService.TableName}` }
    });
  }
